 ...
 *
 */
public class CandleChart extends XYChart<String, Number> {

    SimpleDateFormat sdf = new SimpleDateFormat("HH:mm");
    protected static final Logger logger = Logger.getLogger(CandleChart.class.getName());
    protected int maxBarsToDisplay;
    protected ObservableList<XYChart.Series<String, Number>> dataSeries;
    protected TickData lastBar;
     ...
     */
    public CandleChart(String title, List<TickData> bars) {
     ...
     */
    public CandleChart(String title, List<TickData> bars, int maxBarsToDisplay) {
     ...
     */
    public CandleChart(String title, CategoryAxis xAxis, NumberAxis yAxis, List<TickData> bars, int maxBarsToDisplay) {
        super(xAxis, yAxis);
        this.xAxis = xAxis;
        this.yAxis = yAxis;
        ...
        setTitle(title);
        setAnimated(true);
        getStylesheets().add(getClass().getResource("/css/chart.css").toExternalForm());
        xAxis.setAnimated(true);
        yAxis.setAnimated(true);
        verticalGridLinesVisibleProperty().set(false);
        XYChart.Series<String, Number> series = new XYChart.Series<>();
        List<TickData> sublist = getSubList(bars, maxBarsToDisplay);
        for (TickData bar : sublist) {
            String label = "";
     ...
     */
    public void addBar(TickData bar) {
        ...
        lastBar = new TickData(bar.getDateTime(), bar.getClose(), bar.getClose(), bar.getClose(), bar.getClose(), 0);
    ...
    protected List<TickData> getSubList(List<TickData> bars, int maxBars) {
        List<TickData> sublist;
    ...
    protected void layoutPlotChildren() {
        ...
        for (int seriesIndex = 0; seriesIndex < getData().size(); seriesIndex++) {
            ...
            while (iter.hasNext()) {
                ...
                TickData bar = (TickData) item.getExtraValue();
        ...
        if (xData != null || yData != null) {
            for (Series<String, Number> series : getData()) {
                for (Data<String, Number> data : series.getData()) {
                    ...
                    if (yData != null) {
                        TickData extras = (TickData) data.getExtraValue();
                        if (extras != null) {
    ...
    protected static CandleChart chart;
